/**
 * Created by cnaisin06 on 2017/7/14.
 */
var ZYFILE = {
...
//                        html += '							<img class="upload_image" src="uploadpic/zyupload/skins/images/pics.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
                ...
                var temp=0;
                console.log($("#rapidAddImg"));
                $("#rapidAddImg").on("click", function () {
                    temp++;
                    console.log(temp);
                    if(temp<=1){
                        console.log(temp);
                        $(".add_upload").css({"pointer-events":"none"})
                        $(".add_upload").css({"border":"2px solid gray"})
                    }
                })
...
$(function(){
    ...
    $("#zyupload").zyUpload({
        ...
        //url              :   "https://rest.cnaisin.com:15443/AXGY_OP/commonUpload",
//            url              :   "https://rest.cnaisin.com:8443/AXGY_OP/commonUpload",
            url              :   "http://192.168.31.248:7070/AXGY_OP/commonUpload",
        fileType         :   ["jpg","JPG","jpeg","JPEG","png","PNG","bmp","BMP"],// 上传文件的类型
        fileSize         :   51200000,                // 上传文件的大小
...
//                    input.className="creat";
//                    window.parent.$(".links").append(input);
////                    $("input").attr("value",result.data)
//                }
        },
        onDelete: function(file, files){              // 删除一个文件的回调方法 file:当前删除的文件  files:删除之后的文件
...
//                console.log(response);
            ...
            window.parent.$("#childpic").attr("value",result.data)
//                for(var i=0;i<result.data.length;i++){
//                    window.parent.$(".creat").attr("value",result.data)
//                }
//                window.parent.$("#handpic").attr("value",result.data)
//                $("#single").attr("value",result.data)
//						$("#uploadInf").append("<p>上传成功，文件地址是：" + response + "</p>");
//						$("#uploadInf").append("<p>上传成功</p>");
        ...
        onFailure: function(file, response){          // 文件上传失败的回调方法
            ...
            console.info(file.name);
            $(".file_failure").css({"display":"block"});
            $(".file_success").removeAttr({"display":"block"});
        },
        onComplete: function(response){           	  // 上传完成的回调方法
            console.info("文件上传完成");
            console.info(response);
            window.parent.$(".tipsss").html("已上传")
            $(".mask").hide();
            layer.open({
                skin:"demo-class",
                title:"提示",
